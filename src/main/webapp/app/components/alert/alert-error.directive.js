(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('reportcardApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-reportcardApp-error');
                var entityKey = httpResponse.headers('X-reportcardApp-params');
                if (errorHeader) {
